...
import pandas as pd
df = pd.read_excel('/Users/ken/Desktop/專題/20200428/experiment_data0428/林天立_xlsx/Constrain_weight.xlsx',sheet_name='left')


df['max'] = df[(df.shift(1) <= df) & (df.shift(-1) <= df) & (df > 1.8)]
dfm = df['max']
dfy = df['Y']
...
max = 0
for i in range(len(dfrealindex) - 1) : #找出數據最多
    a = dfrealindex[i + 1] - dfrealindex[i]
    if a > max:
        max = a
...
F = DataFrame(final)
F.to_excel('/Users/ken/Desktop/專題/20200428/experiment_data0428/林天立_output/Constrain_weight_left.xlsx')
print('Finish')